...
$$ 2 $$ 以外の素数は奇数ですから、`(1)`の通り奇数のみを調査対象としています。また、任意の値 $$ a $$ が合成数である場合は、必ず $$ \sqrt{a} $$ より小さい素因数を持つはずです。念のため、これを説明しておきましょう。$$ N $$ が合成数、その素因数を $$ a $$, $$ b $$ とし、$$ N = ab $$(但し $$ 1 \lt a \leq b \land a, b \in \mathbb{Z} $$) とします($$ a $$ が $$ N $$ の最小の素因数であることがポイントです)。つまり、$$ N = ab \geq a^{2} $$ が言えますから $$ \sqrt{N} \geq a $$ とも言えるのです。もしそうでなければ(つまり $$ a $$ も $$ b $$ も $$ \sqrt{N} $$ を超える値であれば)、それは $$ N $$ とはならず、$$ N $$ よりも大きい数になってしまいますね。これを証明 (a) としておきましょう。さて、という事は、$$ a $$ が素数であるかは $$ \sqrt{a} $$ 以下の素数で mod 演算を行えば良い事となります。
...
```
このアルゴリズムは、先ほどの証明 (a) と併用して $$ \sqrt{n} $$ 以下の素数の倍数(自身を除く)を全て篩い落とすことで、素数を得ます。
自分自身を除いた素数の倍数で篩落とすことができれば、最終的に素数だけが残る、というのは直感的ですが、これを少し補足すると、素因数分解の一意性が大きく関わっていることがいえます。
この定理を簡単に言うと、すべての合成数は何らかの素数に割り切ることができるというもので、
[ユークリッドの補題](https://ja.wikipedia.org/wiki/%E3%83%A6%E3%83%BC%E3%82%AF%E3%83%AA%E3%83%83%E3%83%89%E3%81%AE%E8%A3%9C%E9%A1%8C)ともいわれます[^1]。

例えば、$$ n = 30 $$、$$ A $$ が $$ 2 $$ から $$ 30 $$ までの数列だとすると、この数列から次のように篩い落としていきます。
...
5. 7 は $$ \sqrt{30} $$ よりも大きい、証明 (a) よりこれ以上の計算は不必要であるため、終了

上記コードでは、`prime`にビット単位でその値が素数かどうかのフラグを管理させています。
分かりやすさのため、0 と 1 に関するフラグも含めるようにし、初めに`is_prime[0]`と`is_prime[1]`を
0 にしています。<br>
なお、このコードには、改善の余地がまだ残されています。
例えば、`i の倍数で進行`とコメントがある行の`i * 2`について考えて見ましょう。
ここでは、その倍数ずつ進むことで、その素因数の倍数を篩落としています。
結論からいえば、これは無駄です。例えば、2 の倍数のチェック時に、6 はすでに篩落ちています。
しかしながら、3 の倍数のチェック時にも、6 を篩落としてしまいます。
乗法の交換法則によってこれが成り立ちます。
これを防ぐには、その素数の 2 倍から始めるのではなく、2 乗から始めれば良いのです。つまり、`i * i`とします。
例えば 7 の倍数を篩落とすのであれば 49 から開始する 7 の倍数をふるい落すことになります。
それよりも前の 7 の倍数での篩落としは、それよりも前の素数の倍数でチェックできています。<br>
これに加えて、さらに改善する(既知の素数の倍数をスキップする、キャッシュに乗りやすくなるようにループを分割するなど)ことは可能ですが、本項の主題とずれてしまいますので、ここでは一旦置いておきます[^2]。<br>
さて、エラトステネスの篩のベーシックな実装における処理では、
$$ \sqrt{n} $$ 以下の素数の個数分だけこの篩落としが行われることとなります。
つまり、計算量は $$ \sum_{p \lt \sqrt{n}} \dfrac{n}{p} = \dfrac{n}{2} + \dfrac{n}{3} + \dfrac{n}{5} + ... $$ (但し、和は $$ \sqrt{n} $$ 以下の素数全体に対して取る) であると言えます。
