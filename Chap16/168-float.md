...
このとき、$$ 3 =_{2} 011 $$ ですから $$ 011 $$ が指数部の値となりそうです。しかし、指数部の表現は 2 の補数表現を用いない、つまり符号なしとして扱います(大小比較の単純化のためです)。<br>
...
## 16.8.4 精度と誤差
`float`型には、精度と誤差がある事について考察していきます。まず例えば $$ 0.1 $$ といった数値の場合です。
$$ 0.1 $$ を 2 進で考えると、$$ 0.000110011001100\cdots $$ と循環してしまいます。よって有限桁しか扱えないコンピュータ内部では、どこかで打ち切るしかありません。
この打ち切りによって、真の値との誤差が生まれます。<br>
次に、IEEE 754 の形式そのものから生まれる誤差についてです。
ここまでの話から、わかるように、例えば $$ 1 $$ から $$ 2 $$ の間を仮数部の 23 ビットのパターンで表す事ができます。
23 ビットで表せる数はおおよそ 830 万通りですから、$$ 1 $$ から $$ 2 $$ の間を 830 万通りの表現をすることができるということです。
しかしながら、これは浮動小数点数型における強みでありながら、最大の弱みでもあります。
$$ 1 $$ から $$ 2 $$ の間で 830 万通りの表現ができると聞くととても精度の良い数値表現に思えますが、**830 万通りというのは、指数がどれだけ変わっても不変です**。
つまり、例えば $$ 2 $$ から $$ 4 $$ の間にも 830 万通りしかありません。もっと言えば、$$ 20 $$ 億から $$ 40 $$ 億の間にも 830 万通りしかないのです。
これは要するに、**数値が大きくなればなるほど次の数値までの幅が大きくなる**ということです。
