...
\overbrace{\underbrace{00000000000000000000000}_{仮数部({\bf f}raction)}}^{23bit}}^{32bit} $$<br>
$$ 2.00 \cdots $$ と $$ 4.00 \cdots $$ の中間値は、どれだけのパターンで表現することができますでしょうか。指数部を見比べて見るとわかる通り、さきほどと同じく、これまた 23 ビットのパターンで表します。<br>
続いて、$$ 8.000\cdots $$ も見て見ましょう。<br>
$$ 8.00\cdots=\overbrace{
\overbrace{\underbrace{0}_{符号部({\bf s}ign)}}^{1bit}
\overbrace{\underbrace{10000010}_{指数部({\bf e}xponent)}}^{8bit}
\overbrace{\underbrace{00000000000000000000000}_{仮数部({\bf f}raction)}}^{23bit}}^{32bit} $$
...
さて、これらが一体何を示しているか分かりますでしょうか。
これらは、**数が大きくなればなるほど、仮数部における 23 ビットのパターンが引き伸ばされることから、結果として数値間の精度が落ちていく**ということが分かります。
...
このような誤差を、**桁落ち**と言います。
なぜこのような事になってしまうのかは、すぐ分かるはずです。
binary32 の表現方法の必然として、大きな値を扱うためには指数部の値を大きくしていく必要があります。
しかし、指数が 1 つあがれば値は爆発的に増加します。これまで述べてきた通り、仮数部は、その指数値間の値を表現しなければなりません。
これらの事から、値を上げていくと、やがて 23 ビットのみでは(つまり 830 万通りでは)その指数値間の値を桁落ちで保持していくこととなり、最終的には整数単位ですら表現できなくなります。<br>

次のグラフは、$$ 1.00 \cdots $$ から $$ 2.00 \cdots $$ の間の精度を元に点でプロットしたものです。$$ 2.00 - 1.00 $$ より二つの値の差異は $$ 1.00 \cdotss $$ です。これを $$ 2^{23} $$ で分割して表現しますから $$ 1.00 \div 2^{23} = 1.19209e-07 $$ の精度でこの間を表現できることになります(IEEE 574 binary32 実装の`float`型の環境において、`std::numeric_limits<float>::epsilon()`とすることで、同様の値を取得できます)。この値を誤差幅といいます。ここから考えられるように、またグラフからもわかるように、これは点でのプロットを行なったグラフですが、点があまりにも高密度であるためほぼ線のように見えています。
...
二つの距離は $$ 2^{21} - 2^{20} = 2^{20} = 1048576 $$ です。これを $$ 2^{23} $$ で分割しますから $$ 1048576 \div 2^{23} = 0.125 $$ となります。誤差幅が一気に増えましたね。<br>
...
値は当然ながら、このグラフ上の点のどこかでしか表現する事ができないので、点と点の間の数値がもし発生したとしても、このどれかの点上に強制的に合わされます。これが、桁落ちの本質なのです。
