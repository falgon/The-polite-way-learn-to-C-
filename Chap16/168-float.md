...
次のグラフは、$$ 1.00 \cdots $$ から $$ 2.00 \cdots $$ の間の精度を元に点でプロットしたものです。$$ 2.00 - 1.00 $$ より二つの値の差異は $$ 1.00 \cdotss $$ です。これを $$ 2^{23} $$ で分割して表現しますから $$ 1.00 \div 2^{23} = 1.19209e-07 $$ の精度でこの間を表現できることになります(IEEE 754 binary32 実装の`float`型の環境において、`std::numeric_limits<float>::epsilon()`とすることで、同様の値を取得できます)。この値を誤差幅といいます。ここから考えられるように、またグラフからもわかるように、これは点でのプロットを行なったグラフですが、点があまりにも高密度であるためほぼ線のように見えています。
