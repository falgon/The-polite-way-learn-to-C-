...
次のグラフは、$$ 1.00 \cdots $$ から $$ 2.00 \cdots $$ の間の精度を元に点でプロットしたものです。$$ 2.00 - 1.00 $$ より二つの値の差異は $$ 1.00 \cdots $$ です。これを $$ 2^{23} $$ で分割して表現しますから $$ 1.00 \div 2^{23} = 1.19209e-07 $$ の精度でこの間を表現できることになります(IEEE 754 binary32 実装の`float`型の環境において、`std::numeric_limits<float>::epsilon()`とすることで、同様の値を取得できます)。この値を**誤差幅**(また後に説明する絶対誤差)といいます。ここから考えられるように、またグラフからもわかるように、これは点でのプロットを行なったグラフですが、点があまりにも高密度であるためほぼ線のように見えています。
...
1. 理論値 $$ 1 $$ kg の物体の重さを測定したら、$$ 2 $$ kg であった。
2. 理論値 $$ 1000 $$ kg の物体の重さを測定したら、$$ 1001 $$ kg であった。

このとき、誤差幅は 1 と 2 両方とも $$ 1 $$ kg です。しかし、2 よりも 1 の方が大きい誤差であるように思えます。
$$ 1 $$ kg の物体の重さを $$ 1 $$ kg も計り間違えることは重大な誤差です。
しかし、$$ 1024 $$ kg の物体の重さを $$ 1 $$ kg 計り間違えることは、前述した事例よりも重大ではないように思えます。
このように、誤差をスケール(数字の大きさ、桁数）に関係なく、理論値からの幅だけで表すことを、主に**絶対誤差**と言います。
反して、スケールを考慮して理論値からの幅を表すことを**相対誤差**と言います。絶対誤差と相対誤差には次のような関係があります。<br>

$$ 相対誤差 = \dfrac{測定値 - 理論値}{理論値} $$<br>
式中の $$ 測定値 - 理論値 $$ は要するに絶対誤差のことです。<br>
この関係を利用して 1, 2 の事例をそれぞれ相対誤差で表すと、
1 は $$ 1 $$($$ 100% $$) の誤差、2 は $$ 0.001 $$($$ 0.001 % $$)誤差であると言え、両者の誤差の比率を伺うことができます。
...
### 加算
$$ 1 + 1 $$ について考えます。このとき、各 $$ 1 $$ が $$ \pm{e} $$ の絶対誤差を持っている場合、演算結果から取得できる $$ 2 $$ は $$ \pm{2e} $$ の絶対誤差を持っていることになります。<br>
また相対誤差が $$ 0.1 $$ であるとしたときは、
各 $$ 1 $$ は $$ 0.9 $$ または $$ 1.1 $$ の値となります。よって結果は $$ 1.8 $$、$$ 2 $$、$$ 2.2 $$ のうちのどれかとなるでしょう。
つまり足し算の結果のもつ絶対誤差は、元の値の絶対誤差の和となるのです。
