...
## 3.3.5 #if, #ifdef, #ifndef, #elif, defined, #else, __has_include
プリプロセッサ命令では条件分岐を行うことができます。一つずつ　見ていきます。
...
まず、a.hpp内で`int i;`と`i`が定義されています。それを、b.hppとc.hppがインクルードしています。つまり、この時点でb.hppとc.hppの両方で`i`が定義されているという事になります。そしてそれらを`d.hpp`がインクルードしました。この時点で、b.hppの`i`の定義とc.hppの`i`の定義が同じ識別子名となってしまうため、redefinition、再定義というようになってしまうのです。
