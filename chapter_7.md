...
そもそも、ここまででポインターの変数型は全て32ビットであれば4バイト、64ビットであれば8バイトと述べました。同じであるなら何故ポインタの型は何でも良いじゃないか！と思うかもしれませんね。しかしそれにはここまでに述べてきた通り、**何型のポインタなのか**を適切に指定しなければ、アドレスに対して`1`を加算した時、その型の適切なバイト数を進む事ができないのです。
```cpp
#include<climits>
#include<iostream>
int main()
