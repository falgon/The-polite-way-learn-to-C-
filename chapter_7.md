...
一般的に、`null`というワード自体は「空」だとか、「何も初期化されていない」といったニュアンスで使われます。`nullptr`はつまり、「何も初期化されていないポインタ」という意味と捉える事ができます。コードでは、`nullptr`を`int`型のポインター型へ予め代入しています。つまり、`nullptr`は`int`型の数値なのだろうかと考えられるかもしれません。それは実は半分ぐらい合っています。ここまでの知識のみだと、`nullptr`自体の仕組みを理解することはできません。ですので今はとにかく、まずは`nullptr`を代入しておく事で、そのポインターが不正なアドレス値を持っていないかどうかを確認する事ができる、と認識してください。`nullptr`の詳細については、「第11章 テンプレート」にて再度取り上げます。