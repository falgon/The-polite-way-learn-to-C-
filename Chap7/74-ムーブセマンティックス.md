...
まあそれは取り敢えず良しとして、より本題なのは、アンパサンドが二度`int`の後に続いている、rvalue referenceについてです。ここで少し注意して欲しいのが、**`rvr1`はrvalue referenceです。しかし、rvalueではありません。**rvalueは名付けられていませんから、rvalueです。しかし、rvalueのreferenceは名付けられています。よってrvalue referenceそのものはlvalueとなるのです。
