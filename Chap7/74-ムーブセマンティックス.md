...
int a=static_cast<int&&>(i); // rvalue referenceへのキャスト
```
やっとこのコードに戻って来ましたね。なぜrvalue referenceにキャストする事で一時オブジェクトとする事ができるのでしょうか？大切なのは、**rvalueは参照されなくなった途端自動的に破棄される**という鉄則です。参照されなくなった途端に破棄されると言う事は、つまりポインターを定義してそれを勝手に指しても問題ないという事です。
```cpp
void f(int&& a)
{
    
```
なぜrvalueであるならば安全なのでしょうか。それは、もしlvalueだった場合、それは予期されずに今後使えなくなる恐れがあるからです。
```cpp
void f(int* ptr)
{
    int *pp=ptr;
...
int main()
{
    int a=10;
    int* ptr = &a;
    f(ptr); // ptrはいつのまにかnullptrになっている。この後ptrへアクセスするとプログラムはどうなるか分からない
}
しかし、lvalueであっても、今後もうこのlvalueの持っている値は使わないと決めているのであれば、その後そのムーブされたlvalueに対して操作を行う事はないでしょう。

と同じような法則であると言う事です。この概念は、クラスやコンストラクタなどの概念を理解するとより一層理解しやすくなります。それに関しては「第9章 構造体とクラス」にて解説しています。
