...
int main()
{
    ...
    int&& rvr1 = int(); // rvrはrvalue referenceのlvalue
}
```
lvalue referenceではlvalueの参照先を変更する事ができます。しかし、const lvalue referenceではlvalueに対する参照先を変更する事ができないのでrvalueを特別に参照する事ができるようになっています。まあそれは取り敢えず良しとして、より本題なのは、アンパサンドが二度`int`の後に続いている、rvalue referenceのlvalueです。`rvr1`はrvalue referenceなのではないかと思うかもしれませんが、rvalue referenceに対して名付けた時点でそれ自体はlvalueとなるので、rvalueを参照している物(rvalue reference)自体はrvalueではないのです
。何を言っているのかさっぱりですね。rvalue referenceを少し使ってみましょう。
```cpp
#include<iostream>
int main()
{
