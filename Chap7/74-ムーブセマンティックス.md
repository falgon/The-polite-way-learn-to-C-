...
```
では、rvalueはどのように名付けできるのでしょうか。以下のように記述しまう。
```cpp
int main()
{
    const int& rvr = int(); // rvrはconst lvalue reference
    int&& rvr1 = int(); // rvrはrvalue reference
}
```
`const lvalue reference`というまたよくわからないものが出て来ましたね...これについては「第 8 章 定数」で詳しく説明していますが、簡単にいえば`const`は**一切の代入などの変更操作を許さない**という意味のキーワードです。lvalue referenceではlvalueに対する参照先を変更する事ができます。しかし、const lvalue referenceではlvalueに対する参照先を変更する事ができないのです。まあそれは取り敢えず良しとして、より本題なのは、アンパサンドが二度`int`の後に続いている、rvalue referenceです。rvalue referenceを少し使ってみましょう。
```cpp
#include<iostream>
int main()
{
...
42
```
このように、rvalue(上記コードの場合`int (42)`)を`a`という名前で**参照**しそれをlvalueである`b`の初期化値として与えています。そうです、何度か述べていますがつまり単なるrvalueに対するreferenceなのです。

一旦少し以下のコードを見て整理しておきましょう。(どういったものがrvalueとなるのか忘れてしまった場合は、前述したprvalueについての説明コードを見てみてください。)
```cpp
int main()
{
    int a;
...
```
**rvalueは名前がなく参照されなくなった途端自動的に破棄される**ので、lvalueの変数をrvalueにしてしまい、その参照を持つようにすれば、rvalue referenceの変数が参照を辞めた時、自動的に破棄されます。そして、rvalue referenceは殆ど関数へオブジェクトを渡す際にしか使いません。それ以外の部分でrvalue referenceにしたところで用途が特にないからです。よって、関数に対してlvalueをrvalue referenceにした状態でオブジェクトを渡した場合、その関数が終わる頃には自動的に破棄されているので、元々あった値を読み取る事はできないのです。
...
} // i、rvr、a、rvr1が破棄される。aは値は未定義だが領域としてはここまで残っている。
```
