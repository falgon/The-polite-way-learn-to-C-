...
42
```
このように、rvalue(上記コードの場合`int (42)`)を`a`という名前で**参照**しそれをlvalueである`b`の初期化値として与えています。そうです、つまり単なるrvalueに対するreferenceなのです。ここで一旦少し以下のコードを見て整理しておきましょう。(どういったものがrvalueとなるのか忘れてしまった場合は、前述したprvalueについての説明コードを見てみてください。)
```cpp
int main()
{
    int a;
