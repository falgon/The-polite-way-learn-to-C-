...
このrvalue referenceという仕組みは、**値の操作がムーブなのか、それともコピーなのかを型レベルで分別するため**にあるといっても過言ではないでしょう。**`std::move`の適用は、あくまで rvalue reference へ変換するだけなので、それ自体は特にムーブの機能を持っているわけではありません。別の値が初期化される際に、その初期化値として適用するものが rvalue reference だったらムーブの操作をその初期化側で行い、lvalueだったらコピーを行うという区別をする事で、両者の操作を実現しています。よって、`std::move`、つまり rvalue reference への変換というのは、ただのムーブ操作を明示するための、型へのラベル付けのようなものといっても良いでしょう**。
