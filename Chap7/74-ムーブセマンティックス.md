...
```
`const lvalue reference`というまたよくわからないものが出て来ましたね...これについては「第 8 章 定数」で詳しく説明していますが、簡単にいえば`const`は**一切の代入などの変更操作を許さない**という意味のキーワードです。lvalue referenceではlvalueに対する参照先を変更する事ができます。しかし、const lvalue referenceではlvalueに対する参照先を変更する事ができないのです。まあそれは取り敢えず良しとして、より本題なのは、アンパサンドが二度`int`の後に続いている、rvalue referenceです。少し使ってみましょう。
```cpp
#include<iostream>
int main()
{
...
42
```
このように、rvalue(上記コードの場合`int (42)`)を`a`という名前で**参照**しそれをlvalueである`b`の初期化値として与えています。そうです、つまり単なるrvalueに対するreferenceなのです。rvalueに対するreferenceだという事を明確にするため、以下のコードをみてみましょう。(どういったものがrvalueとなるのか忘れてしまった場合は、前述したprvalueについての説明コードを見てみてください。)
```cpp
int main()
{
    int a;
