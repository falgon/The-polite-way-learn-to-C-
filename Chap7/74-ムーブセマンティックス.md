...
簡単に言ってしまえば、このrvalue referenceという仕組みは、**関数に対して送出したデータを関数が終わると同時にその送出したデータを破棄する**ためにある機能と言ってしまっても過言ではないでしょう。それでは以下に、説明して来た内容を実際のコードに表して見ます。
```cpp
void f(int&& rvr)
{
    // 何か処理...
...
int main()
{
    ...
    int&& rvr1 = int(); // int()はrvalueであるがrvr1というrvalue referenceによって参照されているため破棄されない
    rvr1 = int(); // 別のrvalueを参照する事としたためrvr1の初期化時に指定したint() rvalueとは異なるrvalueを参照する。よってrvr1の初期化時に生成されたrvalueは参照者がいなくなったためこのタイミングで破棄される。
}
```
