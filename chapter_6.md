...
ところで、inline展開される事によってどのような利点があるのでしょうか。それは、先ほども述べた通り、関数呼び出しの遅さをなくす事にあります。しかし、欠点もあります。それは、インライン関数は処理をこのように埋め込むわけですから、その分プログラムサイズが増えてしまうのです。
しかし、大抵のコンパイラは、インライン展開する事でプログラムサイズが肥大すると判定するとそれに応じて、インライン展開をせずそのままにしておきます。つまり、普通の関数になるのです。これは自動的に判断されるので、自分で制御することはできません。
最近のコンパイラであれば、わざわざ`inline`キーワードを指定しなくても勝手に高速に動くように最適化してくれる事もあるので、あまり意図的に指定する必要も現代ではないかもしれません。
