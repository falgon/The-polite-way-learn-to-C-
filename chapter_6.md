...
しかし、グローバル変数とは別の手段で、プログラムが終了するまで破棄されない変数を定義する方法があるのです。それが、static変数です。
```cpp
void func()
{
    static int i=1;
...
5
```
static指定子を変数宣言時の型の前に付与する事で、その変数をstatic変数とする事ができます。static変数は、グローバル変数と同様、プログラムが終了するまでその変数は破棄されません。しかし、グローバル変数とは違ってスコープは`func`関数内にありますので、例えば上記コードの場合、`main`関数から変数`i`を見つける事はできません。
